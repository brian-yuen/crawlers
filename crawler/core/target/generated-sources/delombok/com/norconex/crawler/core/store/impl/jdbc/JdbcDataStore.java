...
public class JdbcDataStore<T> implements DataStore<T> {
    ...
    JdbcDataStore(@NonNull JdbcDataStoreEngine engine, @NonNull String storeName, @NonNull Class<? extends T> type) {
        java.util.Objects.requireNonNull(engine, "engine is marked non-null but is null");
        java.util.Objects.requireNonNull(storeName, "storeName is marked non-null but is null");
        java.util.Objects.requireNonNull(type, "type is marked non-null but is null");
        this.engine = engine;
        this.type = type;
        ...
        executeWrite("""
            ...
            WHEN MATCHED THEN
              UPDATE SET
                ...
                t.json = s.json
            """.formatted(adapter.idType(), adapter.modifiedType(), adapter.jsonType()), stmt -> {
    ...
    public Optional<T> find(String id) {
        return executeRead("SELECT id, json FROM <table> WHERE id = ?", stmt -> stmt.setString(1, adapter.serializableId(id)), this::firstObject);
    }

    @Override
    public Optional<T> findFirst() {
        return executeRead("SELECT id, json FROM <table> ORDER BY modified", NO_ARGS, this::firstObject);
    }

    @Override
    public boolean exists(String id) {
        return executeRead("SELECT 1 FROM <table> WHERE id = ?", stmt -> stmt.setString(1, adapter.serializableId(id)), ResultSet::next);
    }

    @Override
    public long count() {
        return executeRead("SELECT count(*) FROM <table>", NO_ARGS, rs -> {
            if (rs.next()) {
                return rs.getLong(1);
            }
            return 0L;
    ...
    public boolean delete(String id) {
        return executeWrite("DELETE FROM <table> WHERE id = ?", stmt -> stmt.setString(1, adapter.serializableId(id))) > 0;
    }

    @Override
    public Optional<T> deleteFirst() {
        Record<T> rec = executeRead("SELECT id, json FROM <table> ORDER BY modified", NO_ARGS, this::firstRecord);
        if (!rec.isEmpty()) {
            delete(rec.id);
        }
        return rec.object;
    ...
    public boolean forEach(BiPredicate<String, T> predicate) {
        return executeRead("SELECT id, json FROM <table>", NO_ARGS, rs -> {
            while (rs.next()) {
                var rec = toRecord(rs);
        ...
        try (var conn = engine.getConnection()) {
            try (var stmt = conn.createStatement()) {
                stmt.executeUpdate("""
                    ...
                    """.formatted(tableName, adapter.idType(), adapter.modifiedType(), adapter.jsonType()));
                stmt.executeUpdate("CREATE INDEX %s_modified_index ON %s(modified)".formatted(tableName, tableName));
    ...
    private <R> R executeRead(String sql, PreparedStatementConsumer psc, ResultSetFunction<R> rsc) {
        try (var conn = engine.getConnection()) {
            try (var stmt = conn.prepareStatement(sql.replace("<table>", tableName))) {
                psc.accept(stmt);
