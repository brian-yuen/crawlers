...
public class DocumentFiltersStage implements Predicate<ImporterPipelineContext> {
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(DocumentFiltersStage.class);
    ...
    public boolean test(ImporterPipelineContext ctx) {
        ...
        for (DocumentFilter filter : filters) {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug(String.format("ACCEPTED document. Reference=%s Filter=%s", ctx.getDocRecord().getReference(), filter));
            }
        }
        if (hasIncludes && !atLeastOneIncludeMatch) {
            ctx.fire(CrawlerEvent.builder().name(CrawlerEvent.REJECTED_FILTER).source(ctx.getCrawler()).crawlDocRecord(ctx.getDocRecord()).subject(filters).message("No \"include\" document filters matched.").build());
            ctx.getDocRecord().setState(CrawlDocState.REJECTED);
            return false;
        }
        return true;
