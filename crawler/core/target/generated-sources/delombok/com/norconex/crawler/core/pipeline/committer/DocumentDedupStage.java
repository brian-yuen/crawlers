...
public class DocumentDedupStage implements Predicate<DocumentPipelineContext> {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(DocumentDedupStage.class);
    ...
    public boolean test(DocumentPipelineContext ctx) {
        ...
        if (ref.isPresent()) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("REJECTED duplicate content checkum found for: {}", docRecord.getReference());
            }
            docRecord.setState(CrawlDocState.REJECTED);
            ctx.fire(CrawlerEvent.builder().name(CrawlerEvent.REJECTED_DUPLICATE).source(ctx.getCrawler()).subject(ref.get()).crawlDocRecord(ctx.getDocRecord()).message("A document with the same content checksum " + "was already processed: " + ref.get()).build());
            return false;
        }
