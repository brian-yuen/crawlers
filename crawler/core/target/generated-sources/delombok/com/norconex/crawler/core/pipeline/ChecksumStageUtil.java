...
public final class ChecksumStageUtil {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(ChecksumStageUtil.class);

    private ChecksumStageUtil() {
    }

    public static boolean resolveMetaChecksum(String newChecksum, DocumentPipelineContext ctx, Object subject) {
        return resolveChecksum(true, newChecksum, ctx, subject);
    }

    public static boolean resolveDocumentChecksum(String newChecksum, DocumentPipelineContext ctx, Object subject) {
        return resolveChecksum(false, newChecksum, ctx, subject);
    }

    // return false if checksum is rejected/unmodified
    private static boolean resolveChecksum(boolean isMeta, String newChecksum, DocumentPipelineContext ctx, Object subject) {
        ...
        // doc, consider as new.
        if (cachedDocInfo == null || CrawlDocState.DELETED.isOneOf(cachedDocInfo.getState())) {
            LOG.debug("ACCEPTED {} checkum (new): Reference={}", type, docInfo.getReference());
        ...
        if (StringUtils.isNotBlank(newChecksum) && Objects.equals(newChecksum, oldChecksum)) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("REJECTED {} checkum (unmodified): Reference={}", type, docInfo.getReference());
            }
            ...
            ctx.fire(CrawlerEvent.builder().name(CrawlerEvent.REJECTED_UNMODIFIED).source(ctx.getCrawler()).crawlDocRecord(ctx.getDocRecord()).subject(subject).message(s.toString()).build());
            return false;
        }
        docInfo.setState(CrawlDocState.MODIFIED);
        LOG.debug("ACCEPTED {} checksum (modified): Reference={}", type, docInfo.getReference());
        return true;
    }
}
