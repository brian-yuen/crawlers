...
public class CrawlDocRecordService implements Closeable {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(CrawlDocRecordService.class);
...
//    private static final String PROP_STAGE = "processingStage";
    ...
    public CrawlDocRecordService(Crawler crawler, Class<? extends CrawlDocRecord> type) {
        this.crawler = Objects.requireNonNull(crawler, "\'crawler\' must not be null.");
    ...
    public boolean prepareForCrawlerStart() {
        ...
        if (resuming) {
            // Active -> Queued
            LOG.debug("Moving any {} active URLs back into queue.", crawler.getId());
            active.forEach((k, v) -> {
                queue.save(k, v);
                return true;
            });
            ...
            if (LOG.isInfoEnabled()) {
                ...
                var totalCount = processedCount + queue.count() + cached.count();
                LOG.info("RESUMING \"{}\" at {} ({}/{}).", crawler.getId(), PercentFormatter.format(processedCount, totalCount, 2, Locale.ENGLISH), processedCount, totalCount);
            }
        } else {
            var storeEngine = crawler.getDataStoreEngine();
            ...
            if (LOG.isInfoEnabled()) {
                ...
                if (cacheCount > 0) {
                    LOG.info("STARTING an incremental crawl from previous {} " + "valid references.", cacheCount);
                } else {
    ...
    public synchronized void processed(CrawlDocRecord docRec) {
        ...
        LOG.debug("Saved processed: {} " + "(Deleted from cache: {}; Deleted from active: {})", docRec.getReference(), cacheDeleted, activeDeleted);
        crawler.getEventManager().fire(CrawlerEvent.builder().name(CrawlerEvent.DOCUMENT_PROCESSED).source(crawler).crawlDocRecord(docRec).build());
    }

    public boolean forEachProcessed(BiPredicate<String, CrawlDocRecord> predicate) {
    ...
    public void queue(CrawlDocRecord docRec) {
        ...
        crawler.getEventManager().fire(CrawlerEvent.builder().name(CrawlerEvent.DOCUMENT_QUEUED).source(crawler).crawlDocRecord(docRec).build());
    }
    ...
    public boolean forEachQueued(BiPredicate<String, CrawlDocRecord> predicate) {
    ...
    public boolean forEachCached(BiPredicate<String, CrawlDocRecord> predicate) {
