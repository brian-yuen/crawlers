...
final class ThreadActionFinalize {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(ThreadActionFinalize.class);
    ...
    static void execute(ThreadActionContext ctx) {
        ...
        if (ctx.doc() == null) {
            ctx.doc(new CrawlDoc(docRecord, CachedInputStream.cache(nullInputStream())));
        }
        var doc = ctx.doc();
        var cachedDocRecord = doc.getCachedDocRecord();
        //--- Ensure we have a state -------------------------------------------
        if (docRecord.getState() == null) {
            LOG.warn("Reference status is unknown for \"{}\". " + "This should not happen. Assuming bad status.", docRecord.getReference());
...
//TODO revisit all the before/after on crawlerImpl
            ofNullable(ctx.crawler().getCrawlerImpl().beforeDocumentFinalizing()).ifPresent(bdf -> bdf.accept(ctx.crawler(), doc));
            ...
            if (!docRecord.getState().isNewOrModified() && cachedDocRecord != null) {
            ...
            LOG.error("Could not finalize processing of: {} ({})", docRecord.getReference(), e.getMessage(), e);
        }
        ...
        try {
            ...
            LOG.error("Could not mark reference as processed: {} ({})", docRecord.getReference(), e.getMessage(), e);
        } finally {
            ofNullable(ctx.crawler().getCrawlerImpl().afterDocumentFinalizing()).ifPresent(adf -> adf.accept(ctx.crawler(), doc));
        }
        try {
    ...
    private static void dealWithBadState(ThreadActionContext ctx) {
        ...
        if (!docRecord.getState().isGoodState() && !docRecord.getState().isOneOf(CrawlDocState.DELETED)) {
            ...
            var strategy = Optional.ofNullable(ctx.crawler().getConfiguration().getSpoiledReferenceStrategizer()).map(srs -> srs.resolveSpoiledReferenceStrategy(ctx.docRecord().getReference(), ctx.docRecord().getState())).orElse(GenericSpoiledReferenceStrategizerConfig.DEFAULT_FALLBACK_STRATEGY);
            if (strategy == SpoiledReferenceStrategy.IGNORE) {
                LOG.debug("Ignoring spoiled reference: {}", docRecord.getReference());
            } else if (strategy == SpoiledReferenceStrategy.DELETE) {
                // Delete if previous state exists and is not already
                // marked as deleted.
                if (cachedDocRecord != null && !cachedDocRecord.getState().isOneOf(CrawlDocState.DELETED)) {
            ...
            if (cachedDocRecord != null && !cachedDocRecord.getState().isOneOf(CrawlDocState.DELETED)) {
                if (!cachedDocRecord.getState().isGoodState()) {
                    ...
                    LOG.debug("""
                        This spoiled reference is being graced once (will be deleted next time if still spoiled): {}""", docRecord.getReference());
                }
            }
        }
    }

    private static void markReferenceVariationsAsProcessed(ThreadActionContext ctx) {
        // Mark original URL as processed
        var originalRef = ctx.docRecord().getOriginalReference();
        var finalRef = ctx.docRecord().getReference();
        if (StringUtils.isNotBlank(originalRef) && ObjectUtils.notEqual(originalRef, finalRef)) {
