...
public class DeleteRejectedEventListener implements EventListener<Event>, Configurable<DeleteRejectedEventListenerConfig> {
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(DeleteRejectedEventListener.class);
    private final DeleteRejectedEventListenerConfig configuration = new DeleteRejectedEventListenerConfig();
    ...
    public void accept(Event event) {
        ...
        if (event.is(CrawlerEvent.CRAWLER_RUN_BEGIN)) {
            ...
            close();
        } else if (event.is(CommitterServiceEvent.COMMITTER_SERVICE_DELETE_END) && !doneCrawling) {
    ...
    private void init(Crawler crawler) {
        ...
        refStore = crawler.getDataStoreEngine().openStore("rejected-refs", Boolean.class);
    }
    ...
    private void storeRejection(CrawlerEvent event) {
        ...
        if (docInfo == null) {
            LOG.warn("Listening for reference rejections on a crawler event " + "that has no reference: {}", event.getName());
            return;
        }
    ...
    private void commitDeletions(Crawler crawler) {
        if (LOG.isInfoEnabled()) {
            LOG.info("Committing {} rejected references for deletion...", refStore.count());
        }
        refStore.forEach((ref, sent) -> {
            if (Boolean.FALSE.equals(sent)) {
                crawler.getCommitterService().delete(new CrawlDoc(new CrawlDocRecord(ref), CachedInputStream.cache(new NullInputStream())));
            }
            return true;
        });
    ...
    public boolean equals(final java.lang.Object o) {
        ...
        if (!(o instanceof DeleteRejectedEventListener)) return false;
        final DeleteRejectedEventListener other = (DeleteRejectedEventListener) o;
    ...
    protected boolean canEqual(final java.lang.Object other) {
        return other instanceof DeleteRejectedEventListener;
    }
    ...
    public java.lang.String toString() {
        return "DeleteRejectedEventListener(configuration=" + this.getConfiguration() + ")";
    }

    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public DeleteRejectedEventListenerConfig getConfiguration() {
