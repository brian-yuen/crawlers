...
public class DateFormatTransformer extends BaseDocumentHandler implements Configurable<DateFormatTransformerConfig> {
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(DateFormatTransformer.class);
    private final DateFormatTransformerConfig configuration = new DateFormatTransformerConfig();

    @Override
    public void handle(DocContext docCtx) throws IOException {
        validateArguments();
        var fromDates = docCtx.metadata().getStrings(configuration.getFromField());
        ...
        if (StringUtils.isBlank(configuration.getToField())) {
            PropertySetter.REPLACE.apply(docCtx.metadata(), configuration.getFromField(), toDates);
        } else {
            PropertySetter.orAppend(configuration.getOnSet()).apply(docCtx.metadata(), configuration.getToField(), toDates);
        }
    }
    ...
    private String formatDate(String fromDate) {
        ...
        for (String fromFormat : formats) {
            try {
                var fromZdt = ZonedDateTimeParser.builder().format(nullIfEpoch(fromFormat)).locale(ofNullable(configuration.getFromLocale()).orElse(Locale.ENGLISH)).build().parse(fromDate);
                if (nullIfEpoch(configuration.getToFormat()) == null) {
                    return Long.toString(fromZdt.toInstant().toEpochMilli());
                }
                var toDate = fromZdt.format(DateTimeFormatter.ofPattern(configuration.getToFormat()).localizedBy(ofNullable(configuration.getToLocale()).orElse(Locale.ENGLISH)));
                if (StringUtils.isNotBlank(toDate)) {
                    return toDate;
                }
            } catch (DateTimeException e) {
                LOG.debug("Could not parse date \'{}\' with format \'{}\' " + "and locale {}.", fromDate, fromFormat, configuration.getFromLocale());
            }
        }
        return null;
    }
    ...
    private void validateArguments() {
        if (StringUtils.isBlank(configuration.getFromField())) {
            throw new IllegalArgumentException("\"fromField\" cannot be empty.");
        }
        if (StringUtils.isBlank(configuration.getFromField()) && StringUtils.isBlank(configuration.getToField())) {
            throw new IllegalArgumentException("One of \"fromField\" or \"toField\" is required.");
        }
    }

    private static String nullIfEpoch(String format) {
        return "EPOCH".equalsIgnoreCase(StringUtils.trimToEmpty(format)) ? null : format;
    }
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public DateFormatTransformerConfig getConfiguration() {
    ...
    public boolean equals(final java.lang.Object o) {
        ...
        if (!(o instanceof DateFormatTransformer)) return false;
        final DateFormatTransformer other = (DateFormatTransformer) o;
    ...
    protected boolean canEqual(final java.lang.Object other) {
        return other instanceof DateFormatTransformer;
    }
    ...
    public java.lang.String toString() {
        return "DateFormatTransformer(super=" + super.toString() + ", configuration=" + this.getConfiguration() + ")";
    }
}
