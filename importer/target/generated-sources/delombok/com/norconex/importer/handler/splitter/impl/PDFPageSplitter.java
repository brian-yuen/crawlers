...
public class PDFPageSplitter extends AbstractDocumentSplitter<PDFPageSplitterConfig> {
    ...
    private final PDFPageSplitterConfig configuration = new PDFPageSplitterConfig();
    ...
    public void split(DocContext docCtx) throws DocumentHandlerException {
        ...
        if (!MatchUtil.matchesContentType(configuration.getContentTypeMatcher(), docCtx.docRecord()) || (docCtx.metadata().getInteger(DOC_PDF_PAGE_NO, 0) > 0)) {
            return;
        }
        try (var document = PDDocument.load(docCtx.input().asInputStream())) {
            ...
            for (PDDocument page : splittedDocuments) {
                pageNo++;
                var pageRef = docCtx.reference() + trimToEmpty(configuration.getReferencePagePrefix()) + pageNo;
                // metadata
                var pageMeta = new Properties();
                pageMeta.loadFromMap(docCtx.metadata());
                var pageInfo = new DocRecord(pageRef);
                pageMeta.set(DocMetadata.EMBEDDED_REFERENCE, Integer.toString(pageNo));
                ...
                var pageDoc = new Doc(pageInfo, docCtx.streamFactory().newInputStream(os.toInputStream()), pageMeta);
                pageDocs.add(pageDoc);
            }
        } catch (IOException e) {
            throw new DocumentHandlerException("Could not split PDF: " + docCtx.reference(), e);
        }
    }
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public PDFPageSplitterConfig getConfiguration() {
    ...
    public boolean equals(final java.lang.Object o) {
        ...
        if (!(o instanceof PDFPageSplitter)) return false;
        final PDFPageSplitter other = (PDFPageSplitter) o;
    ...
    protected boolean canEqual(final java.lang.Object other) {
        return other instanceof PDFPageSplitter;
    }
    ...
    public java.lang.String toString() {
        return "PDFPageSplitter(super=" + super.toString() + ", configuration=" + this.getConfiguration() + ")";
    }
}
