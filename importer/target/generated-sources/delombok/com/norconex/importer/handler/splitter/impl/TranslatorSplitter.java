...
public class TranslatorSplitter extends AbstractDocumentSplitter<TranslatorSplitterConfig> {
    private final TranslatorSplitterConfig configuration = new TranslatorSplitterConfig();
    ...
    public TranslatorSplitter() {
        translators.put(TranslatorSplitterConfig.API_MICROSOFT, new TranslatorStrategy() {
            @Override
            ...
            public void validateProperties() throws DocumentHandlerException {
                if (StringUtils.isAnyBlank(configuration.getClientId(), configuration.getClientSecret())) {
                    throw new DocumentHandlerException("Both clientId and clientSecret must be specified.");
                }
            }
        });
        translators.put(TranslatorSplitterConfig.API_GOOGLE, new TranslatorStrategy() {
            @Override
            public Translator createTranslator() {
            ...
            public void validateProperties() throws DocumentHandlerException {
                if (StringUtils.isAnyBlank(configuration.getApiKey())) {
                    throw new DocumentHandlerException("apiKey must be specified.");
                }
            }
        });
        translators.put(TranslatorSplitterConfig.API_LINGO24, new TranslatorStrategy() {
            @Override
            public Translator createTranslator() {
            ...
            public void validateProperties() throws DocumentHandlerException {
                if (StringUtils.isAnyBlank(configuration.getUserKey())) {
                    throw new DocumentHandlerException("userKey must be specified.");
                }
            }
        });
        translators.put(TranslatorSplitterConfig.API_MOSES, new TranslatorStrategy() {
            @Override
            public Translator createTranslator() {
                return new MosesTranslator(configuration.getSmtPath(), configuration.getScriptPath());
            }
            @Override
            public void validateProperties() throws DocumentHandlerException {
                if (StringUtils.isAnyBlank(configuration.getSmtPath(), configuration.getScriptPath())) {
                    throw new DocumentHandlerException("Both smtPath and scriptPath must be specified.");
                }
            }
        });
        translators.put(TranslatorSplitterConfig.API_YANDEX, new TranslatorStrategy() {
            @Override
            public Translator createTranslator() {
    ...
    public void split(DocContext docCtx) throws DocumentHandlerException {
        ...
        if (configuration.getFieldMatcher().isSet()) {
            ...
            try {
                ...
                throw new DocumentHandlerException("Could not translate document: " + docCtx.reference(), e);
            }
        } else {
            // Body
            ...
            try (
                ...
                var cachedInput = input instanceof CachedInputStream cis ? cis : docCtx.streamFactory().newInputStream(input)) {
                for (String lang : configuration.getTargetLanguages()) {
                    var translatedDoc = translateDocumentFromStream(docCtx, cachedInput, lang);
    ...
    private Doc translateDocumentFromStream(DocContext docCtx, CachedInputStream cachedInput, String targetLang) throws DocumentHandlerException {
        if (Objects.equals(configuration.getSourceLanguage(), targetLang)) {
            return null;
        }
        cachedInput.rewind();
        try (var reader = new TextReader(new InputStreamReader(cachedInput, StandardCharsets.UTF_8), getTranslatorStrategy().getReadSize())) {
            return translateDocumentFromReader(docCtx, targetLang, reader);
        } catch (Exception e) {
            var extra = "";
            if (TranslatorSplitterConfig.API_GOOGLE.equals(configuration.getApi()) && e instanceof IndexOutOfBoundsException) {
                extra = " \"apiKey\" is likely invalid.";
            }
            throw new DocumentHandlerException("Translation failed form \"%s\" to \"%s\" for: \"%s\". %s".formatted(configuration.getSourceLanguage(), targetLang, docCtx.reference(), extra), e);
        }
    }

    private Doc translateDocumentFromReader(DocContext docCtx, String targetLang, TextReader reader) throws Exception {
        ...
        try (var childContent = docCtx.streamFactory().newOuputStream()) {
            ...
            /*NOOP*/childInput = childContent.getInputStream();
        }
    ...
    private Properties translateFields(DocContext docCtx, String targetLang) throws Exception {
        var translator = getTranslatorStrategy().getTranslator();
        var sourceLang = getResolvedSourceLanguage(docCtx);
        var childMeta = new Properties();
        ...
        var fieldsToTranslate = List.copyOf(docCtx.metadata().matchKeys(fieldMatcher).keySet());
    ...
    private TranslatorStrategy getTranslatorStrategy() {
        ...
        if (strategy == null) {
            throw new ImporterRuntimeException("Unsupported translation api: " + configuration.getApi());
        }
        return strategy;
    }

    private void validateProperties(DocContext doc) throws DocumentHandlerException {
        if (StringUtils.isBlank(configuration.getApi())) {
            throw new DocumentHandlerException("Must specify a translation api.");
        }
        if (configuration.getTargetLanguages().isEmpty()) {
            throw new DocumentHandlerException("No translation target language(s) specified.");
        }
        var sourceLang = getResolvedSourceLanguage(doc);
        if (sourceLang == null || Language.fromString(sourceLang) == null) {
            throw new DocumentHandlerException("Unsupported source language: \"" + sourceLang + "\"");
        }
        for (String targetLang : configuration.getTargetLanguages()) {
            if (Language.fromString(targetLang) == null) {
                throw new DocumentHandlerException("Unsupported target language: \"" + targetLang + "\"");
            }
        }
    ...
    private String getResolvedSourceLanguage(DocContext docCtx) {
        var lang = docCtx.metadata().getString(configuration.getSourceLanguageField());
    ...
    private static abstract class TranslatorStrategy {
        ...
        public abstract void validateProperties() throws DocumentHandlerException;
    }

    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public TranslatorSplitterConfig getConfiguration() {
    ...
    public boolean equals(final java.lang.Object o) {
        ...
        if (!(o instanceof TranslatorSplitter)) return false;
        final TranslatorSplitter other = (TranslatorSplitter) o;
    ...
    protected boolean canEqual(final java.lang.Object other) {
        return other instanceof TranslatorSplitter;
    }
    ...
    public java.lang.String toString() {
        return "TranslatorSplitter(super=" + super.toString() + ", configuration=" + this.getConfiguration() + ")";
    }
}
