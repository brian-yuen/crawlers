...
class RecursiveEmbeddedMerger extends ParserDecorator {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(RecursiveEmbeddedMerger.class);
    ...
    public RecursiveEmbeddedMerger(Parser parser, Writer writer, DocContext docCtx, EmbeddedConfig embeddedConfig) {
        super(parser);
        this.writer = writer;
        this.docCtx = docCtx;
    ...
    public void parse(InputStream stream, ContentHandler handler, Metadata tikaMeta, ParseContext context) throws IOException, SAXException, TikaException {
        ...
        if (maxDepth >= 0 && embedDepth > maxDepth) {
            LOG.debug("Skipping embedded document {} which is over max " + "depth: {}", maxDepth, resName);
            return;
        }
        // Don't parse if unwanted embedded of parent content type
        var parentType = Optional.ofNullable(typesHierarchy.peekLast()).map(ContentType::toBaseTypeString).orElse(null);
        if (!isMasterDoc && TextMatcher.anyMatches(embeddedConfig.getSkipEmbeddedOfContentTypes(), parentType)) {
            LOG.debug("Skipping embedded document {} " + "of parent content type: {}.", resName, parentType);
            return;
        }
        ...
        if (TextMatcher.anyMatches(embeddedConfig.getSkipEmbeddedContentTypes(), currentType.toBaseTypeString())) {
            LOG.debug("Skipping embedded document {} " + "with content type: {}.", resName, currentType);
            return;
        }
        // All good, parse.
        typesHierarchy.add(currentType);
        super.parse(stream, new BodyContentHandler(writer), tikaMeta, context);
