...
public class TextBetweenTransformer extends BaseDocumentHandler implements Configurable<TextBetweenTransformerConfig> {
    private final TextBetweenTransformerConfig configuration = new TextBetweenTransformerConfig();

    @Override
    public void handle(DocContext docCtx) throws IOException {
        for (TextBetweenOperation op : configuration.getOperations()) {
            ListValuedMap<String, String> opExtractions = MultiMapUtils.newListValuedHashMap();
            ChunkedTextReader.builder().charset(configuration.getSourceCharset()).fieldMatcher(op.getFieldMatcher()).maxChunkSize(configuration.getMaxReadSize()).build().read(docCtx, chunk -> {
                opExtractions.putAll(firstNonBlank(op.getToField(), chunk.getField()), doExtractTextBetween(op, chunk.getText()));
                return true;
            });
            opExtractions.asMap().forEach((fld, vals) -> {
                PropertySetter.orAppend(op.getOnSet()).apply(docCtx.metadata(), fld, vals);
            });
        }
    }

    private List<String> doExtractTextBetween(TextBetweenOperation op, String text) {
        ...
        while (leftMatch.find()) {
            ...
            if (op.isInclusive()) {
                matches.add(new ImmutablePair<>(leftMatch.start(), rightMatch.end()));
            } else {
                matches.add(new ImmutablePair<>(leftMatch.end(), rightMatch.start()));
            }
        }
        List<String> values = new ArrayList<>();
        for (var i = matches.size() - 1; i >= 0; i--) {
            var matchPair = matches.get(i);
            var value = text.substring(matchPair.getLeft(), matchPair.getRight());
            if (value != null) {
                values.add(value);
            }
        }
    ...
    @lombok.Generated
    public TextBetweenTransformer() {
    }

    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public TextBetweenTransformerConfig getConfiguration() {
    ...
    public boolean equals(final java.lang.Object o) {
        ...
        if (!(o instanceof TextBetweenTransformer)) return false;
        final TextBetweenTransformer other = (TextBetweenTransformer) o;
    ...
    protected boolean canEqual(final java.lang.Object other) {
        return other instanceof TextBetweenTransformer;
    }
    ...
    public java.lang.String toString() {
        return "TextBetweenTransformer(super=" + super.toString() + ", configuration=" + this.getConfiguration() + ")";
    }
}
