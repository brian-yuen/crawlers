...
public class DomTransformer extends BaseDocumentHandler implements Configurable<DomTransformerConfig> {
    private final DomTransformerConfig configuration = new DomTransformerConfig();

    @Override
    public void handle(DocContext docCtx) throws IOException {
        // only proceed if we are dealing with a supported content type
        if (!configuration.getContentTypeMatcher().matches(docCtx.docRecord().getContentType().toString())) {
            return;
        }
        // disable chunking to not break the DOM.
        ChunkedTextReader.builder().charset(configuration.getSourceCharset()).fieldMatcher(configuration.getFieldMatcher()).maxChunkSize(-1).build().read(docCtx, chunk -> {
    ...
    private void applyOperations(DocContext docCtx, TextChunk chunk) throws IOException {
        if (StringUtils.isBlank(chunk.getText())) {
            return;
        }
        var doc = Jsoup.parse(chunk.getText(), docCtx.reference(), DomUtil.toJSoupParser(configuration.getParser()));
        ...
        for (DomOperation op : configuration.getOperations()) {
            ...
            if (isNotBlank(op.getToField()) && !extractedValues.isEmpty()) {
                PropertySetter.orAppend(op.getOnSet()).apply(docCtx.metadata(), op.getToField(), extractedValues);
            }
        }
    ...
    private void applyOperation(Document doc, DomOperation op, List<String> extractions, List<String> preserveOnly) {
    ...
    private List<String> handleExtractAndDelete(Document doc, DomOperation op) {
        List<String> extractedValues = new ArrayList<>();
        ...
        for (Element elm : elms) {
            ...
            var matches = ((value != null) && (op.isMatchBlanks() || !StringUtils.isBlank(value)));
            if (matches) {
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public DomTransformerConfig getConfiguration() {
    ...
    public boolean equals(final java.lang.Object o) {
        ...
        if (!(o instanceof DomTransformer)) return false;
        final DomTransformer other = (DomTransformer) o;
    ...
    protected boolean canEqual(final java.lang.Object other) {
        return other instanceof DomTransformer;
    }
    ...
    public java.lang.String toString() {
        return "DomTransformer(super=" + super.toString() + ", configuration=" + this.getConfiguration() + ")";
    }
}
