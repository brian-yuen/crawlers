...
public class CsvSplitter extends AbstractDocumentSplitter<CsvSplitterConfig> {
    ...
    public void split(DocContext docCtx) throws IOException {
        try {
            ...
            if (!configuration.getFieldMatcher().isSet()) {
                docCtx.childDocs().addAll(doSplitDocument(docCtx, docCtx.input().asInputStream(), count));
                return;
            }
            // Fields
            var docs = docCtx.childDocs();
            docCtx.metadata().matchKeys(configuration.getFieldMatcher()).forEach((k, vals) -> vals.forEach(row -> docs.addAll(doSplitDocument(docCtx, new ByteArrayInputStream(row.getBytes()), count))));
        } catch (Exception e) {
            throw new DocumentHandlerException("Could not split document: " + docCtx.reference(), e);
        }
    }

    private List<Doc> doSplitDocument(DocContext doc, InputStream input, MutableInt count) {
        List<Doc> rows = new ArrayList<>();
        var parser = new CSVParserBuilder().withSeparator(configuration.getSeparatorCharacter()).withQuoteChar(configuration.getQuoteCharacter()).withEscapeChar(configuration.getEscapeCharacter()).build();
        ...
        // quotes per line, etc.
        try (var csvreader = new CSVReaderBuilder(new InputStreamReader(input, StandardCharsets.UTF_8)).withSkipLines(configuration.getLinesToSkip()).withCSVParser(parser).build()) {
            String[] rowColumns;
            ...
            while ((rowColumns = csvreader.readNextSilently()) != null) {
                ...
                if (cnt == 1 && configuration.isUseFirstRowAsFields()) {
                    ...
                    rows.add(parseRow(doc, rowColumns, colNames, childEmbedRef));
                }
            }
        } catch (IOException e) {
    ...
    private Doc parseRow(DocContext docCtx, String[] rowColumns, String[] colNames, String childEmbedRef) {
        ...
        for (var i = 0; i < rowColumns.length; i++) {
            ...
            if (isColumnMatchingNameOrPosition(colName, colPos, isBlank(refColumn) ? List.of() : List.of(refColumn))) {
                childEmbedRef = colValue;
            }
            // If a content column, add it to content
            if (isColumnMatchingNameOrPosition(colName, colPos, configuration.getContentColumns())) {
        ...
        if (contentStr.length() > 0) {
            content = docCtx.streamFactory().newInputStream(contentStr.toString());
        } else {
    ...
    private boolean isColumnMatchingNameOrPosition(String colName, int colPosition, List<String> namesOrPossToMatch) {
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public CsvSplitterConfig getConfiguration() {
    ...
    public boolean equals(final java.lang.Object o) {
        ...
        if (!(o instanceof CsvSplitter)) return false;
        final CsvSplitter other = (CsvSplitter) o;
    ...
    protected boolean canEqual(final java.lang.Object other) {
        return other instanceof CsvSplitter;
    }
    ...
    public java.lang.String toString() {
        return "CsvSplitter(super=" + super.toString() + ", configuration=" + this.getConfiguration() + ")";
    }
}
