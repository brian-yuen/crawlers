...
class RecursiveEmbeddedSplitter extends ParserDecorator {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(RecursiveEmbeddedSplitter.class);
    ...
    public RecursiveEmbeddedSplitter(Parser parser, DocContext docCtx, List<Doc> embeddedDocs, EmbeddedConfig embeddedConfig) {
        super(parser);
        this.docCtx = docCtx;
    ...
    @Override
    public void parse(InputStream input, ContentHandler handler, Metadata tikaMeta, ParseContext context) throws IOException, SAXException, TikaException {
        ...
        if (maxDepth >= 0 && embedDepth > maxDepth) {
            LOG.warn("Skipping embedded document {} which is over max " + "depth: {}", maxDepth, resName);
            return;
        }
        // Don't parse if unwanted embedded of parent content type
        var parentType = docCtx.docRecord().getContentType().toBaseTypeString();
        if (TextMatcher.anyMatches(embeddedConfig.getSkipEmbeddedOfContentTypes(), parentType)) {
            LOG.debug("Skipping embedded document {} " + "of parent content type: {}.", tikaMeta.get(TikaCoreProperties.RESOURCE_NAME_KEY), parentType);
            return;
        }
        ...
        if (TextMatcher.anyMatches(embeddedConfig.getSkipEmbeddedContentTypes(), embedContentType.toBaseTypeString())) {
            LOG.debug("Skipping embedded document {} " + "with content type: {}.", tikaMeta.get(TikaCoreProperties.RESOURCE_NAME_KEY), embedContentType);
            return;
        }
        embedCount++;
        var embedMeta = new Properties();
        TikaUtil.metadataToProperties(tikaMeta, embedMeta);
        var embedRecord = resolveEmbeddedResourceName(tikaMeta, embedMeta, embedCount);
        embedRecord.setEmbeddedParentReferences(docCtx.docRecord().getEmbeddedParentReferences());
        // Read the steam into cache for reuse since Tika will
        // close the original stream on us causing exceptions later.
        try (var embedOutput = docCtx.streamFactory().newOuputStream()) {
            IOUtils.copy(input, embedOutput);
            var embedInput = embedOutput.getInputStream();
            embedRecord.addEmbeddedParentReference(docCtx.reference());
            var embedDoc = new Doc(embedRecord, embedInput, embedMeta);
            embeddedDocs.add(embedDoc);
        }
    }

    private DocRecord resolveEmbeddedResourceName(Metadata tikaMeta, Properties embedMeta, int embedCount) {
        new DocRecord();
        docCtx.reference();
        String name;
        ...
        if (StringUtils.isNotBlank(name)) {
            ...
            if (ct != null) {
                return docRecord("embedded-" + embedCount + "." + ct.getExtension(), embedMeta, "file-file");
            }
        }
        // Default... we could not find any name so make a unique one.
        return docRecord("embedded-" + embedCount + ".unknown", embedMeta, "unknown");
    }

    private DocRecord docRecord(String embedRef, Properties embedMeta, String embedType) {
