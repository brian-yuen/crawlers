...
public class ExternalTransformer extends BaseDocumentHandler implements Configurable<ExternalTransformerConfig> {
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(ExternalTransformer.class);
    private final ExternalTransformerConfig configuration = new ExternalTransformerConfig();
    ...
    public void handle(DocContext docCtx) throws IOException {
        ...
        try {
            ...
            if (files.hasOutputMetaFile()) {
                try (Reader outputMetaReader = Files.newBufferedReader(files.outputMetaFile)) {
            ...
            externalMeta.forEach((k, v) -> PropertySetter.orAppend(configuration.getOnSet()).apply(docCtx.metadata(), k, v));
        } finally {
            files.deleteAll();
        }
    }

    private int executeCommand(final String cmd, final ArgFiles files, final Properties metadata, final InputStream input, final OutputStream output) throws IOException {
        var systemCommand = new SystemCommand(cmd);
        systemCommand.setEnvironmentVariables(configuration.getEnvironmentVariables());
        ...
        try {
            ...
            throw new IOException("External transformer failed. Command: " + configuration.getCommand(), e);
        }
    }
    ...
    private void writeLine(String line, OutputStream output) {
        try {
            ...
            throw new ImporterRuntimeException("Could not write to output", e);
        }
    }

    private synchronized void extractMetaFromFile(Reader reader, Properties metadata) {
    ...
    private synchronized void extractMetaFromLine(String line, Properties metadata) {
        RegexFieldValueExtractor.extractFieldValues(metadata, line, configuration.getExtractionPatterns().toArray(RegexFieldValueExtractor.EMPTY_ARRAY));
    }

    private Path createTempFile(Object stream, String name, String suffix) throws IOException {
        Path tempDirectory;
    ...
    private String resolveInputToken(String cmd, ArgFiles files, InputStream is) throws IOException {
        if (!cmd.contains(TOKEN_INPUT) || is == null) {
            return cmd;
        }
        var newCmd = cmd;
        files.inputFile = createTempFile(is, "input", ".tmp");
        newCmd = StringUtils.replace(newCmd, TOKEN_INPUT, files.inputFile.toAbsolutePath().toString());
        try {
    ...
    private String resolveInputMetaToken(String cmd, ArgFiles files, InputStream is, Properties meta) throws IOException {
        if (!cmd.contains(TOKEN_INPUT_META)) {
            return cmd;
        }
        var newCmd = cmd;
        files.inputMetaFile = createTempFile(is, "input-meta", "." + StringUtils.defaultIfBlank(configuration.getMetadataInputFormat(), META_FORMAT_JSON));
        newCmd = StringUtils.replace(newCmd, TOKEN_INPUT_META, files.inputMetaFile.toAbsolutePath().toString());
    ...
    private String resolveOutputToken(String cmd, ArgFiles files, OutputStream os) throws IOException {
        if (!cmd.contains(TOKEN_OUTPUT) || os == null) {
            return cmd;
        }
        var newCmd = cmd;
        files.outputFile = createTempFile(os, "output", ".tmp");
        return StringUtils.replace(newCmd, TOKEN_OUTPUT, files.outputFile.toAbsolutePath().toString());
    }

    private String resolveOutputMetaToken(String cmd, ArgFiles files, OutputStream os) throws IOException {
        if (!cmd.contains(TOKEN_OUTPUT_META)) {
            return cmd;
        }
        var newCmd = cmd;
        files.outputMetaFile = createTempFile(os, "output-meta", "." + StringUtils.defaultIfBlank(configuration.getMetadataOutputFormat(), ".tmp"));
        return StringUtils.replace(newCmd, TOKEN_OUTPUT_META, files.outputMetaFile.toAbsolutePath().toString());
    }
        ...
        static void delete(Path file) {
            if (file != null) {
                try {
                    ...
                    LOG.warn("Could not delete temporary file: " + file.toAbsolutePath(), e);
                }
            }
        }
    }
    ...
    public boolean equals(final java.lang.Object o) {
        ...
        if (!(o instanceof ExternalTransformer)) return false;
        final ExternalTransformer other = (ExternalTransformer) o;
    ...
    protected boolean canEqual(final java.lang.Object other) {
        return other instanceof ExternalTransformer;
    }
    ...
    public java.lang.String toString() {
        return "ExternalTransformer(super=" + super.toString() + ", configuration=" + this.getConfiguration() + ")";
    }
}
