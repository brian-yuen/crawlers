...
public class TruncateTransformer extends BaseDocumentHandler implements Configurable<TruncateTransformerConfig> {
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(TruncateTransformer.class);
    private static final int BODY_CHUNK_SIZE = DataUnit.KB.toBytes(1).intValue();
    private final TruncateTransformerConfig configuration = new TruncateTransformerConfig();
    ...
    public void doBody(DocContext docCtx) throws IOException {
        ...
        var charRead = 0;
        try (
            var reader = IOUtils.buffer(docCtx.input().asReader());
            var writer = docCtx.output().asWriter()) {
            for (var text = readString(reader); text != null; text = readString(reader)) {
            ...
            var txt = ofNullable(secondToLastChunk).orElse("") + ofNullable(lastChunk).orElse("");
            ...
            if (StringUtils.isNotBlank(configuration.getToField())) {
                // set on target field
                PropertySetter.orAppend(configuration.getOnSet()).apply(docCtx.metadata(), configuration.getToField(), txt);
            } else {
                writer.write(txt);
            }
        }
    ...
    public void doFields(DocContext docCtx) {
        ...
        for (Entry<String, List<String>> en : docCtx.metadata().matchKeys(configuration.getFieldMatcher()).entrySet()) {
            ...
            for (String sourceValue : sourceValues) {
                var truncValue = truncate(sourceValue, configuration.getMaxLength());
                targetValues.add(truncValue);
                if (LOG.isDebugEnabled() && !Objects.equals(truncValue, sourceValue)) {
                    LOG.debug("\"{}\" value truncated to \"{}\".", fromField, truncValue);
                }
            }
            ...
            if (StringUtils.isBlank(configuration.getToField())) {
                ...
                PropertySetter.REPLACE.apply(docCtx.metadata(), fromField, targetValues);
            } else {
        ...
        if (StringUtils.isNotBlank(configuration.getToField())) {
            // set on target field
            PropertySetter.orAppend(configuration.getOnSet()).apply(docCtx.metadata(), configuration.getToField(), allTargetValues);
        }
    }
    ...
    private String truncate(String value, int maxLength) {
        ...
        if (configuration.isAppendHash()) {
            return StringUtil.truncateWithHash(value, maxLength, configuration.getSuffix());
        }
        if (StringUtils.isNotEmpty(configuration.getSuffix())) {
            return StringUtils.abbreviate(value, configuration.getSuffix(), maxLength);
        }
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public TruncateTransformerConfig getConfiguration() {
    ...
    public boolean equals(final java.lang.Object o) {
        ...
        if (!(o instanceof TruncateTransformer)) return false;
        final TruncateTransformer other = (TruncateTransformer) o;
    ...
    protected boolean canEqual(final java.lang.Object other) {
        return other instanceof TruncateTransformer;
    }
    ...
    public java.lang.String toString() {
        return "TruncateTransformer(super=" + super.toString() + ", configuration=" + this.getConfiguration() + ")";
    }
}
