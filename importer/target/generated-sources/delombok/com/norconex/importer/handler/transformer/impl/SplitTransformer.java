...
public class SplitTransformer extends BaseDocumentHandler implements Configurable<SplitTransformerConfig> {
    private final SplitTransformerConfig configuration = new SplitTransformerConfig();
    ...
    private void splitContent(SplitOperation op, DocContext docCtx) throws IOException {
        ...
        PropertySetter.orAppend(op.getOnSet()).apply(docCtx.metadata(), op.getToField(), targetValues);
    }

    private void splitMetadata(SplitOperation split, DocContext docCtx) {
        List<String> allTargetValues = new ArrayList<>();
        for (Entry<String, List<String>> en : docCtx.metadata().matchKeys(split.getFieldMatcher()).entrySet()) {
            var fromField = en.getKey();
            var sourceValues = en.getValue();
            List<String> targetValues = new ArrayList<>();
            for (String sourceValue : sourceValues) {
                if (split.isSeparatorRegex()) {
                    targetValues.addAll(regexSplit(sourceValue, split.getSeparator()));
                } else {
                    targetValues.addAll(regularSplit(sourceValue, split.getSeparator()));
                }
            }
        ...
        if (StringUtils.isNotBlank(split.getToField())) {
            // set on target field
            PropertySetter.orAppend(split.getOnSet()).apply(docCtx.metadata(), split.getToField(), allTargetValues);
        }
    }
    ...
    private List<String> regularSplit(String metaValue, String separator) {
        return Arrays.asList(StringUtils.splitByWholeSeparator(metaValue, separator));
    }
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public SplitTransformerConfig getConfiguration() {
    ...
    public boolean equals(final java.lang.Object o) {
        ...
        if (!(o instanceof SplitTransformer)) return false;
        final SplitTransformer other = (SplitTransformer) o;
    ...
    protected boolean canEqual(final java.lang.Object other) {
        return other instanceof SplitTransformer;
    }
    ...
    public java.lang.String toString() {
        return "SplitTransformer(super=" + super.toString() + ", configuration=" + this.getConfiguration() + ")";
    }
}
