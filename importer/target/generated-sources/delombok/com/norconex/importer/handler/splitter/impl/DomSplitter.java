...
public class DomSplitter extends AbstractDocumentSplitter<DomSplitterConfig> {
    ...
    public void split(DocContext docCtx) throws DocumentHandlerException {
        ...
        if (configuration.getFieldMatcher().isSet()) {
            docCtx.childDocs();
            docCtx.metadata().matchKeys(configuration.getFieldMatcher()).forEach((k, vals) -> vals.forEach(v -> parse(docCtx, Jsoup.parse(v, docCtx.reference(), DomUtil.toJSoupParser(configuration.getParser())))));
        } else {
            // Body
            try {
                var inputCharset = CharsetUtil.firstNonNullOrUTF8(docCtx.parseState(), configuration.getSourceCharset(), docCtx.docRecord().getCharset());
                var soupDoc = Jsoup.parse(docCtx.input().asInputStream(), inputCharset.toString(), docCtx.reference(), DomUtil.toJSoupParser(configuration.getParser()));
                parse(docCtx, soupDoc);
            } catch (IOException e) {
                throw new DocumentHandlerException("Cannot parse document into a DOM-tree.", e);
            }
        }
    }
    ...
    private void parse(DocContext docCtx, Document soupDoc) {
        ...
        for (Element elm : elms) {
            ...
            if (childContent.length() > 0) {
                content = docCtx.streamFactory().newInputStream(childContent);
            } else {
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public DomSplitterConfig getConfiguration() {
    ...
    public boolean equals(final java.lang.Object o) {
        ...
        if (!(o instanceof DomSplitter)) return false;
        final DomSplitter other = (DomSplitter) o;
    ...
    protected boolean canEqual(final java.lang.Object other) {
        return other instanceof DomSplitter;
    }
    ...
    public java.lang.String toString() {
        return "DomSplitter(super=" + super.toString() + ", configuration=" + this.getConfiguration() + ")";
    }
}
