...
public class Importer {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(Importer.class);
    ...
    private static final InheritableThreadLocal<Importer> INSTANCE = new InheritableThreadLocal<>();
...
//        parseHandler = new ImporterParseHandler(this);
    ...
    public ImporterResponse importDocument(ImporterRequest req) {
        try {
            ...
            return new ImporterResponse().setReference(req.getReference()).setStatus(ImporterResponse.Status.ERROR).setException(new ImporterException("Failed to import document for request: " + req, e)).setDescription(e.getLocalizedMessage());
        }
    }
...
//            }
            ...
            //--- Response Processor ---
            if (response.getParentResponse() == null && !configuration.getResponseProcessors().isEmpty()) {
                processResponse(response);
            }
            return response;
        } catch (IOException | ImporterRuntimeException e) {
            LOG.warn("Could not import document: {}", document, e);
            return new ImporterResponse().setStatus(Status.ERROR).setDoc(document).setReference(document.getReference()).setException(new ImporterException("Could not import document: " + document, e));
        } finally {
            destroyHandlersOnce();
        }
    }
    ...
    private synchronized void initializeHandlersOnce() {
        BeanUtil.visitAll(configuration.getHandler(), t -> {
            try {
                t.init();
            } catch (IOException e) {
                throw new ImporterRuntimeException("Coult not initialize handler: " + t, e);
            }
    ...
    private void prepareDocumentForImporting(Doc document) {
        ...
        if (ct == null || StringUtils.isBlank(ct.toString())) {
            try {
                ct = ContentTypeDetector.detect(document.getInputStream(), document.getReference());
            } catch (IOException e) {
                LOG.warn("Could not detect content type. Defaulting to " + "\"application/octet-stream\".", e);
        ...
        try {
            var encoding = CharsetDetector.builder().priorityCharset(docRecord::getCharset).fallbackCharset((Charset) null).build().detect(document);
            docRecord.setCharset(encoding);
        } catch (IOException e) {
            LOG.debug("Problem detecting encoding for: {}", docRecord.getReference(), e);
        }
        ...
        if (docRecord.getCharset() != null) {
            meta.set(DocMetadata.CONTENT_ENCODING, docRecord.getCharset().toString());
        }
    }
    ...
    private Doc toDocument(ImporterRequest req) throws ImporterException {
        ...
        if (req.getInputStream() != null) {
            // From input stream
            is = CachedInputStream.cache(req.getInputStream(), requestStreamFactory);
        } else if (req.getFile() != null) {
            // From file
            if (!req.getFile().toFile().isFile()) {
                throw new ImporterException("File does not exists or is not a file: " + req.getFile().toAbsolutePath());
            }
            try {
                is = requestStreamFactory.newInputStream(new FileInputStream(req.getFile().toFile()));
            } catch (IOException e) {
                throw new ImporterException("Could not import file: " + req.getFile().toAbsolutePath(), e);
            }
    ...
    private synchronized void ensureRequestStreamFactory() {
        ...
        requestStreamFactory = new CachedStreamFactory((int) configuration.getMaxMemoryPool(), (int) configuration.getMaxMemoryInstance(), configuration.getTempDir());
    }
...
//    }
    private void processResponse(ImporterResponse response) {
        for (ImporterResponseProcessor proc : configuration.getResponseProcessors()) {
            //MAYBE: do something with return response?
            proc.processImporterResponse(response);
        }
    }

    private ImporterResponse executeHandlers(Doc doc, List<Doc> childDocsHolder) throws ImporterException {
        var resp = new ImporterResponse().setDoc(doc).setReference(doc.getReference());
        if (configuration.getHandler() == null) {
            return resp.setStatus(Status.SUCCESS);
        }
        var ctx = DocContext.builder().doc(doc).eventManager(eventManager).build();
        try {
            ...
            try {
                ...
                LOG.error("Could not flush document stream for {}", ctx.reference(), e);
            }
        }
        childDocsHolder.addAll(ctx.childDocs());
        if (ctx.isRejected()) {
            return resp.setStatus(Status.REJECTED).setRejectCause(ctx.rejectedBy()).setDescription(Objects.toString(ctx.rejectedBy(), null));
        }
...
//        return PASSING_FILTER_STATUS;
    ...
    public java.lang.String toString() {
        return "Importer(configuration=" + this.getConfiguration() + ")";
    }
    ...
    public boolean equals(final java.lang.Object o) {
        ...
        if (!(o instanceof Importer)) return false;
        final Importer other = (Importer) o;
    ...
    protected boolean canEqual(final java.lang.Object other) {
        return other instanceof Importer;
    }
