...
public class CSVFileCommitter extends AbstractFSCommitter<CSVPrinter, CSVFileCommitterConfig> {
    public static final int DEFAULT_TRUNCATE_AT = 5096;
    private final CSVFileCommitterConfig configuration = new CSVFileCommitterConfig();
    ...
    protected CSVPrinter createDocWriter(Writer writer) throws IOException {
        var builder = Builder.create(configuration.getFormat() == null ? CSVFormat.newFormat(',') : new EnumConverter().toType(configuration.getFormat().toString(), CSVFormat.Predefined.class).getFormat());
    ...
    private void printHeaders(CSVPrinter csv) throws IOException {
        ...
        if (StringUtils.isNotBlank(configuration.getTypeHeader()) || !configuration.isSplitUpsertDelete()) {
            csv.print(isNotBlank(configuration.getTypeHeader()) ? configuration.getTypeHeader() : "type");
        }
    ...
    @Override
    protected void writeUpsert(CSVPrinter csv, UpsertRequest upsertRequest) throws IOException {
        if (isNotBlank(configuration.getTypeHeader()) || !configuration.isSplitUpsertDelete()) {
            csv.print("upsert");
        }
        for (CSVColumn col : configuration.getColumns()) {
            ...
            if (StringUtils.isBlank(field)) {
                ...
                value = StringUtils.join(upsertRequest.getMetadata().getStrings(field), configuration.getMultiValueJoinDelimiter());
            }
    ...
    @Override
    protected void writeDelete(CSVPrinter csv, DeleteRequest deleteRequest) throws IOException {
        if (isNotBlank(configuration.getTypeHeader()) || !configuration.isSplitUpsertDelete()) {
            csv.print("delete");
        }
        for (CSVColumn col : configuration.getColumns()) {
            ...
            if (StringUtils.isNotBlank(field)) {
                value = StringUtils.join(deleteRequest.getMetadata().getStrings(field), configuration.getMultiValueJoinDelimiter());
            }
            value = truncate(value, col.getTruncateAt());
            csv.print(StringUtils.trimToEmpty(value));
        }
    ...
    @Override
    protected void closeDocWriter(CSVPrinter csv) throws IOException {
        if (csv != null) {
            csv.flush();
            csv.close();
        }
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public CSVFileCommitterConfig getConfiguration() {
    ...
    public boolean equals(final java.lang.Object o) {
        ...
        if (!(o instanceof CSVFileCommitter)) return false;
        final CSVFileCommitter other = (CSVFileCommitter) o;
    ...
    protected boolean canEqual(final java.lang.Object other) {
        return other instanceof CSVFileCommitter;
    }
    ...
    public java.lang.String toString() {
        return "CSVFileCommitter(super=" + super.toString() + ", configuration=" + this.getConfiguration() + ")";
    }
