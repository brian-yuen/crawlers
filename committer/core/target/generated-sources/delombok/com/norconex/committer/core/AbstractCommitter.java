...
public abstract class AbstractCommitter<T extends BaseCommitterConfig> implements Committer, Configurable<T> {
    @JsonIgnore
    private CommitterContext committerContext;

    @Override
    public final void init(@NonNull CommitterContext committerContext) throws CommitterException {
        java.util.Objects.requireNonNull(committerContext, "committerContext is marked non-null but is null");
    ...
    public boolean accept(CommitterRequest request) throws CommitterException {
        try {
            if (getConfiguration().getRestrictions().isEmpty() || getConfiguration().getRestrictions().matches(request.getMetadata())) {
    ...
    protected void applyFieldMappings(CommitterRequest req) {
        ...
        for (Entry<String, List<String>> en : req.getMetadata().entrySet()) {
            ...
            if (getConfiguration().getFieldMappings().containsKey(fromField)) {
                var toField = getConfiguration().getFieldMappings().get(fromField);
    ...
    protected final void fireDebug(String name, CommitterRequest req) {
        fire(CommitterEvent.builder().name(name).source(this).request(req).build(), Level.DEBUG);
    }
    ...
    protected final void fireInfo(String name, CommitterRequest req) {
        fire(CommitterEvent.builder().name(name).source(this).request(req).build(), Level.INFO);
    }
    ...
    protected final void fireError(String name, CommitterRequest req, Exception e) {
        fire(CommitterEvent.builder().name(name).source(this).request(req).exception(e).build(), Level.ERROR);
    }

    private void fire(CommitterEvent e, Level level) {
        Optional.ofNullable(committerContext).map(CommitterContext::getEventManager).ifPresent(em -> em.fire(e, level));
    }
    ...
    public boolean equals(final java.lang.Object o) {
        ...
        if (!(o instanceof AbstractCommitter)) return false;
        final AbstractCommitter<?> other = (AbstractCommitter<?>) o;
    ...
    protected boolean canEqual(final java.lang.Object other) {
        return other instanceof AbstractCommitter;
    }
