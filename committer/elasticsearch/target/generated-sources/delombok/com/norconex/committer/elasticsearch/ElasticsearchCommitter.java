...
public class ElasticsearchCommitter extends AbstractBatchCommitter<ElasticsearchCommitterConfig> {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(ElasticsearchCommitter.class);
    public static final String ELASTICSEARCH_ID_FIELD = "_id";
    private RestClient client;
    private Sniffer sniffer;
    private final ElasticsearchCommitterConfig configuration = new ElasticsearchCommitterConfig();
    ...
    private String extractId(CommitterRequest req) throws CommitterException {
        return fixBadIdValue(CommitterUtil.extractSourceIdValue(req, configuration.getSourceIdField()));
    }
    ...
    protected void commitBatch(Iterator<CommitterRequest> it) throws CommitterException {
        ...
        try {
            ...
            throw new CommitterException("Could not commit JSON batch to Elasticsearch.", e);
        }
    }
    ...
    private void handleResponse(Response response) throws IOException, CommitterException {
        var respEntity = response.getEntity();
        if (respEntity != null) {
            var responseAsString = IOUtils.toString(respEntity.getContent(), StandardCharsets.UTF_8);
            if (LOG.isTraceEnabled()) {
            ...
            if (StringUtils.substring(responseAsString, 0, 100).contains("\"errors\":true")) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Elasticsearch response status: {}", response.getStatusLine());
        }
        if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
            throw new CommitterException("Invalid HTTP response: " + response.getStatusLine());
        }
    }
    ...
    private void appendUpsertRequest(StringBuilder json, UpsertRequest req) throws CommitterException {
        CommitterUtil.applyTargetContent(req, configuration.getTargetContentField());
        json.append("{\"index\":{");
        ...
        for (Entry<String, List<String>> entry : req.getMetadata().entrySet()) {
            ...
            field = StringUtils.replace(field, ".", configuration.getDotReplacement());
    ...
    private void appendDeleteRequest(StringBuilder json, DeleteRequest req) throws CommitterException {
        json.append("{\"delete\":{");
        append(json, "_index", configuration.getIndexName());
        if (StringUtils.isNotBlank(configuration.getTypeName())) {
            append(json.append(','), "_type", configuration.getTypeName());
    ...
    private void append(StringBuilder json, String field, List<String> values) {
        ...
        json.append('\"').append(StringEscapeUtils.escapeJson(field)).append("\":[");
    ...
    private void appendValue(StringBuilder json, String field, String value) {
        if (configuration.getJsonFieldsPattern() != null && configuration.getJsonFieldsPattern().matches(field)) {
            json.append(value);
        } else {
    ...
    private String fixBadIdValue(String value) throws CommitterException {
        ...
        if (configuration.isFixBadIds() && value.getBytes(StandardCharsets.UTF_8).length > 512) {
            String v;
            try {
                v = StringUtil.truncateBytesWithHash(value, StandardCharsets.UTF_8, 512, "!");
            } catch (CharacterCodingException e) {
                LOG.error("""
                    Bad id detected (too long), but could not be truncated properly by byte size. Will truncate based on characters size instead, which may not work on IDs containing multi-byte characters.""");
    ...
    protected RestClient createRestClient() {
        ...
        builder.setFailureListener(new FailureListener() {
            ...
            public void onFailure(Node node) {
                LOG.error("Failure occured on node: \"{}\". Check node logs.", node.getName());
            }
        });
        builder.setRequestConfigCallback(rcb -> rcb.setConnectTimeout((int) configuration.getConnectionTimeout().toMillis()).setSocketTimeout((int) configuration.getSocketTimeout().toMillis()));
        var credentials = configuration.getCredentials();
        if (credentials.isSet()) {
            CredentialsProvider credsProvider = new BasicCredentialsProvider();
            credsProvider.setCredentials(AuthScope.ANY, new UsernamePasswordCredentials(credentials.getUsername(), EncryptionUtil.decrypt(credentials.getPassword(), credentials.getPasswordKey())));
            builder.setHttpClientConfigCallback(b -> b.setDefaultCredentialsProvider(credsProvider));
        }
    ...
    protected Sniffer createSniffer(RestClient client) {
        ...
        if (!configuration.getNodes().isEmpty() && configuration.getNodes().get(0).startsWith("https:")) {
            NodesSniffer nodesSniffer = new ElasticsearchNodesSniffer(client, ElasticsearchNodesSniffer.DEFAULT_SNIFF_REQUEST_TIMEOUT, ElasticsearchNodesSniffer.Scheme.HTTPS);
            return Sniffer.builder(client).setNodesSniffer(nodesSniffer).build();
        }
    ...
    public boolean equals(final java.lang.Object o) {
        ...
        if (!(o instanceof ElasticsearchCommitter)) return false;
        final ElasticsearchCommitter other = (ElasticsearchCommitter) o;
    ...
    protected boolean canEqual(final java.lang.Object other) {
        return other instanceof ElasticsearchCommitter;
    }
    ...
    public java.lang.String toString() {
        return "ElasticsearchCommitter(super=" + super.toString() + ", configuration=" + this.getConfiguration() + ")";
    }

    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public ElasticsearchCommitterConfig getConfiguration() {
