...
public class ApacheKafkaCommitter extends AbstractBatchCommitter<ApacheKafkaCommitterConfig> {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(ApacheKafkaCommitter.class);
    private static final String EXCEPTION_MSG_INVALID_CONFIG = "Invalid configuration. Both `topicName` and `bootstrapServers` " + "are required.";
    ...
    private final ApacheKafkaCommitterConfig configuration = new ApacheKafkaCommitterConfig();
    private KafkaProducer<String, String> producer;
    private KafkaAdmin kafkaAdmin;

    @Override
    protected void initBatchCommitter() throws CommitterException {
        if (StringUtils.isBlank(configuration.getTopicName()) || StringUtils.isBlank(configuration.getBootstrapServers())) {
        ...
        if (configuration.isCreateTopic()) {
            if (configuration.getPartitions() == 0 || configuration.getReplicationFactor() == 0) {
                var msg = String.format("%s=true requires these settings be also set. %s, %s", CREATE_TOPIC_CONFIG, NUM_OF_PARTITIONS_CONFIG, REPLICATION_FACTOR_CONFIG);
                throw new CommitterException(msg);
            }
            LOG.info("Ensuring topic `{}` exists in Kafka", configuration.getTopicName());
            kafkaAdmin.ensureTopicExists(configuration.getTopicName(), configuration.getPartitions(), configuration.getReplicationFactor());
        } else if (!kafkaAdmin.isTopicExists(configuration.getTopicName())) {
            var msg = String.format("Topic `%s` does not exist in Kafka. " + "Either create the topic manually or set `%s` to true.", configuration.getTopicName(), CREATE_TOPIC_CONFIG);
            LOG.error(msg);
            throw new CommitterException(msg);
        }
    }
        ...
        try {
            while (it.hasNext()) {
                ...
                if (req instanceof UpsertRequest upsert) {
                    ...
                    var rec = new ProducerRecord<>(configuration.getTopicName(), upsert.getReference(), json.toString());
                    producer.send(rec);
                    docCountUpserts++;
                    json.setLength(0);
                } else if (req instanceof DeleteRequest delete) {
                    var json = new StringBuilder();
                    var rec = new ProducerRecord<String, String>(configuration.getTopicName(), delete.getReference(), null);
                    producer.send(rec);
                    docCountDeletes++;
                    json.setLength(0);
                } else {
            ...
            if (docCountUpserts > 0) {
                LOG.info("Sent {} upsert commit operation(s) to Apache Kafka.", docCountUpserts);
            }
            if (docCountDeletes > 0) {
                LOG.info("Sent {} delete commit operation(s) to Apache Kafka.", docCountDeletes);
            }
    ...
    private void appendUpsertRequest(StringBuilder json, UpsertRequest upsert) throws CommitterException {
    ...
    private void appendFieldAndValue(StringBuilder json, String field, String value) {
        json.append("\"").append(StringEscapeUtils.escapeJson(field)).append("\":");
        appendValue(json, value);
    }
    ...
    public boolean equals(final java.lang.Object o) {
        ...
        if (!(o instanceof ApacheKafkaCommitter)) return false;
        final ApacheKafkaCommitter other = (ApacheKafkaCommitter) o;
    ...
    protected boolean canEqual(final java.lang.Object other) {
        return other instanceof ApacheKafkaCommitter;
    }
    ...
    public java.lang.String toString() {
        return "ApacheKafkaCommitter(super=" + super.toString() + ", configuration=" + this.getConfiguration() + ")";
    }

    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public ApacheKafkaCommitterConfig getConfiguration() {
