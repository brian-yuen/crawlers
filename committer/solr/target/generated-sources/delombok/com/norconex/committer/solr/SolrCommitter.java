...
public class SolrCommitter extends AbstractBatchCommitter<SolrCommitterConfig> {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(SolrCommitter.class);
    private SolrClient solrClient;
    private final SolrCommitterConfig configuration = new SolrCommitterConfig();

    @Override
    protected void initBatchCommitter() throws CommitterException {
        solrClient = ObjectUtils.defaultIfNull(configuration.getSolrClientType(), SolrClientType.HTTP2).create(configuration.getSolrURL());
    }
    ...
    protected void commitBatch(Iterator<CommitterRequest> it) throws CommitterException {
        ...
        try {
            ...
            throw new CommitterException("Cannot push document batch to Solr.", e);
        }
    }
    ...
    protected void pushSolrRequest(UpdateRequest solrBatchRequest) throws SolrServerException, IOException, CommitterException {
        if (configuration.getCredentials().isSet()) {
            solrBatchRequest.setBasicAuthCredentials(configuration.getCredentials().getUsername(), EncryptionUtil.decryptPassword(configuration.getCredentials()));
        }
        for (Entry<String, String> entry : configuration.getUpdateUrlParams().entrySet()) {
    ...
    protected void addSolrUpsertRequest(UpdateRequest solrBatchRequest, UpsertRequest committerRequest) throws CommitterException {
        CommitterUtil.applyTargetId(committerRequest, configuration.getSourceIdField(), configuration.getTargetIdField());
        CommitterUtil.applyTargetContent(committerRequest, configuration.getTargetContentField());
        solrBatchRequest.add(buildSolrDocument(committerRequest.getMetadata()));
    }

    protected void addSolrDeleteRequest(UpdateRequest solrBatchRequest, DeleteRequest committerRequest) {
        CommitterUtil.applyTargetId(committerRequest, configuration.getSourceIdField(), configuration.getTargetIdField());
    ...
    private void handleResponse(UpdateResponse response) throws CommitterException {
        handleResponse(response.getResponse());
    }

    private void handleResponse(NamedList<Object> response) throws CommitterException {
        @SuppressWarnings("unchecked")
        var headers = (NamedList<Object>) response.get("responseHeader");
        if (headers == null) {
            throw new CommitterException("No response headers obtained from Solr request. " + "Response: " + response);
        }
        var status = Objects.toString(headers.get("status"), null);
        if (!"0".equals(status)) {
            throw new CommitterException("Invalid Solr response status: " + status);
        }
    }
    ...
    public boolean equals(final java.lang.Object o) {
        ...
        if (!(o instanceof SolrCommitter)) return false;
        final SolrCommitter other = (SolrCommitter) o;
    ...
    protected boolean canEqual(final java.lang.Object other) {
        return other instanceof SolrCommitter;
    }
    ...
    public java.lang.String toString() {
        return "SolrCommitter(super=" + super.toString() + ", configuration=" + this.getConfiguration() + ")";
    }

    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public SolrCommitterConfig getConfiguration() {
